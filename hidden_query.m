function X = hidden_query(A, seed, p, K, eta, beta, maxiter, tol)
...
%   - seed: index of the query seed nodes (one or more)
...
n = size(A, 1);
if nargin <= 2, p = 0.008; end
if nargin <= 3, K = 10; end
if nargin <= 4, Eta = [1.5*ones(1, round(K/2)), 1.25*ones(1, K-round(K/2))]; end
if nargin <= 5, beta = 2.5; end
if nargin <= 6, maxiter = 100; end
if nargin <= 7, tol = 1e-5; end
if nargin >= 5 && length(eta) == 1, Eta = eta*ones(1,K); end
...
lb = X(:, 2); lb(seed) = 0.9; ub = ones(n, 1);  % lower and upper bound for X(:,1) 
